...
/* TB6612 truth table, per channel (STBY high):
 *
 *   IN1  IN2   PWM      output
 *    0    0     x       stop / coast (both outputs off)
 *    1    0    duty     forward
 *    0    1    duty     reverse
 *    1    1     x       short brake (both outputs low)
 */
...
{
    /* STBY low first, so the bridges stay off while the direction pins are
     * still floating from reset — otherwise a motor can twitch at power-on. */
...
void TB6612_SetMotor(Motor_Side_t side, int8_t speed)
{
    ...
    /* INT8_MIN has no positive counterpart, so clamp before negating. */
    ...
    if (magnitude == 0u) {
        motor_set_bridge(m, false, false);      /* coast */
    } else {
...
void TB6612_Drive(Car_Direction_t dir, uint8_t left_speed, uint8_t right_speed)
{
    ...
    switch (dir) {
        ...
        case CAR_TURN_LEFT:      /* spin in place, anticlockwise */
            TB6612_SetMotor(MOTOR_LEFT,  (int8_t)-l);
            TB6612_SetMotor(MOTOR_RIGHT,  r);
            break;

        case CAR_TURN_RIGHT:     /* spin in place, clockwise */
            TB6612_SetMotor(MOTOR_LEFT,   l);
            TB6612_SetMotor(MOTOR_RIGHT, (int8_t)-r);
            break;
